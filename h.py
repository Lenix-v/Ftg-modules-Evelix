#    Friendly uTelegram (telegram userbot)
#    Copyrighst (C) 2018-2019 The Authors
...
class AFKMod(loader.Module):
    ...
    strings = {"name": "Evelix",
               "gone": "<b>Включить автответчик</b>",
               "back": "<b>Отключить автответчик</b>",
               "afk": "<b>({}).</b>",
               "afk_reason": "<b>({}).\nReason:</b> <i>{}</i>"}
    ...
    async def afkcmd(self, message):
        ...
        if utils.get_args_raw(message):
            self._db.set(__name__, "aff", utils.get_args_raw(message))
        else:
            self._db.set(__name__, "aff", True)
    ...
    async def unafkcmd(self, message):
        ...
        self._db.set(__name__, "aff", False)
    ...
    async def watcher(self, message):
        ...
        if message.mentioned or getattr(message.to_id, "user_id", None) == self._me.id:
            ...
            if afk_state is True:
                ret = self.strings("aff", message).format(diff)
    ...
    def get_afk(self):
        return self._db.get(__name__, "aff", False)
